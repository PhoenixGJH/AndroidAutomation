...
def convert(cc):
    ...
    while i < strLen:
        try:
            if charPYStr().index(cc[i]) >= 0:
                strRes += ftPYStr()[charPYStr().index(cc[i])]
        except ValueError:
            strRes += cc[i]
...
def create_new_string(file, new_file):
    if os.path.exists(file):
        # tree = ET.parse("strings.xml")
        tree = ET.parse(file)
        root = tree.getroot()
        for node in root.iter('string'):
            new_str = convert(node.text)
            node.text = str(new_str)

        # tree.write('strings2.xml', "utf-8")
        tree.write(new_file, "utf-8")


# directory_root = '%s%s%s' % ('..', os.path.sep, 'MFAssistant')
directory_root = os.path.join(os.getcwd())
for directory, subdir_list, file_list in os.walk(directory_root):
    if directory.find("main\\res\\values") != -1:
        new_path = directory.split("values")[0] + "\\values-zh-rTW"
        is_success = makedirs(new_path)
        if is_success:
            print("old: " + directory + "\\strings.xml" +
                  "\nNew: " + new_path + "\\strings.xml")
            create_new_string(directory + "\\strings.xml",
                              new_path + "\\strings.xml")

    # for name in subdir_list:
    #     if name.find("values") != -1:
    #         print("findValues: " + directory)
    #         for file_name in file_list:
    #             if file_name.find("strings.xml") != -1 and directory.find():
    #                 print("findStrings: " + directory)
    #                 new_path = makedirs(directory.split(
    #                     "values")[0] + "\\values-zh-rTW")
    #                 create_new_string(directory + "\\" + file_name,
    #                                   new_path + "\\strings.xml")
